...
def _downsample_mask(mask: np.ndarray, spacing: Tuple[float, float, float], target_max_dim: int = 96) -> Tuple[np.ndarray, Tuple[float, float, float]]:
...
def build_mesh_from_mask(mask: np.ndarray, spacing: Tuple[float, float, float]) -> Dict[str, object]:
    if not np.any(mask):
        return {
            "vertices": [],
            "faces": [],
    ...
    mesh_mask, mesh_spacing = _downsample_mask(mask, spacing)
